...
const props = defineProps(["item", "index", "translations", "swiper", "words", "totalItems"]),
    isMobile = ref(window.innerWidth <= 768),
    lang = inject('lang');


function nextSlide() {
  props.swiper.value.slideNext();
}

function prevSlide() {
  props.swiper.value.slidePrev();
}
  ...
  <div class="relative">
    ...
    >
      <div class="rounded-[10px] overflow-hidden "
           :class="{'rounded-none' : (index === 0 && isMobile.value) }">
        <div class="md:h-56 h-32">
          ...
          >
          <div v-else
               class="flex flex-col relative justify-center md:px-4 pl-4 pr-16 w-full h-full bg-[#05203C]">
            <h2 class="text-white font-normal text-sm md:text-md">{{ translations[0].subtitle }}</h2>
            <h1 class="text-white font-bold text-4xl md:text-[58px] leading-1">{{ translations[0].title }}</h1>
    ...
    <!--right arrow-->
    <button
        v-show="index !== 0 /*&& index !== points.length -1*/"
